...
int top(int  *B,
		int *C,int test0,int test1)
{
	matrix_t A[matrix_size][matrix_size];
	matrix_t L[matrix_size][matrix_size];
	memcpy(&A[0][0], const_cast<int*>(B), sizeof(int) * matrix_size * matrix_size);
#pragma HLS INTERFACE s_axilite port=test1 bundle=CTRL_BUS
#pragma HLS INTERFACE s_axilite port=test0 bundle=CTRL_BUS

#pragma HLS INTERFACE m_axi port=B offset=slave bundle=master_bus0
#pragma HLS INTERFACE m_axi port=C offset=slave bundle=master_bus1
...
#pragma HLS stream variable=feedout3 depth=5
	...
	collector(
		...
		feedout3,
	       	L);
	memcpy(&C, &L[0][0], sizeof(float) * matrix_size * matrix_size);
	test0 = A[0][0];
	test1 = L[0][0];
	return 0;
}
